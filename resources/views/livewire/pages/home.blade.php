    ...
    <section style="margin-top: 200px;">
        <div class="container">
            ...
            <div class="text-center my-4">
                <button id="load-more-btn" class="btn btn-primary">{{ __('Load More') }}</button>
            </div>
        </div>
    </section>
...
<script async>
    ...
    let loadedCount = 0; // Tracks how many items have been loaded

    // Fetch data from the JSON file
    fetch('/assets/json/socmed.json')
        .then(response => response.json())
        .then(jsonData => {
            data = jsonData; // Store the fetched data
            displayItems(); // Display the first set of items
        })
        .catch(error => console.error('Error fetching JSON:', error));

    // Function to display items in batches of 4
    function displayItems() {
        const container = document.getElementById("embeds-container");
        const loadMoreBtn = document.getElementById("load-more-btn");
        const itemsToShow = 4; // Number of items to display per batch
        const maxItems = 12; // Maximum number of items to display

        // Create a temporary container to store new items
        const tempContainer = document.createElement("div");
        tempContainer.classList.add("d-flex", "flex-wrap", "gap-1", "justify-content-center"); // Add flexbox classes


        // Generate HTML for the next set of items
        for (let i = loadedCount; i < loadedCount + itemsToShow && i < data.length; i++) {
            const item = data[i];
            const embedStyle = `
                max-width: 400px; 
                min-width: 300px; 
            ...
            if (item.platform === "tiktok") {
                tempContainer.innerHTML += `
                    <div class="flex-grow-1 mb-4" style="justify-items:center">
        ...
        // Append only the new rows to the main container
        container.appendChild(tempContainer);

        // Update the loaded count
        loadedCount += itemsToShow;

        // Dynamically initialize TikTok for new content only
        const tiktokScript = document.createElement('script');
        tiktokScript.src = "https://www.tiktok.com/embed.js";
        tiktokScript.async = true;
        tempContainer.appendChild(tiktokScript);

        // Dynamically reinitialize Instagram embeds for new content only
        if (window.instgrm && window.instgrm.Embeds) {
            window.instgrm.Embeds.process(tempContainer);
        }

        // Hide the "Load More" button if all items are displayed
        if (loadedCount >= maxItems || loadedCount >= data.length) {
            loadMoreBtn.style.display = "none";
        }
    }

    // Event listener for the "Load More" button
    document.getElementById("load-more-btn").addEventListener("click", displayItems);
</script>
